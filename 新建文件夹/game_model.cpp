...
void GameModel::createGame(int row, int col, int mineCount, GameLevel level)
{
    ...
    for(int i = 0; i < mRow; i++)
    {
        ...
        {
            MineBlock mineBlock;
            mineBlock.curState = BlockState::UN_DIG;
            mineBlock.valueFlag = 0; // 默认为0
...
{
    // 正常方块且没有被翻开过，标记为已挖
    if(gameMap[m][n].valueFlag > 0 && gameMap[m][n].curState == BlockState::UN_DIG)
    {
        gameMap[m][n].curState = BlockState::DIGGED;
    }

    // 遇到空白块（数字0）就递归挖雷，如果踩雷就爆炸，游戏结束
    if(gameMap[m][n].valueFlag == 0 && gameMap[m][n].curState == BlockState::UN_DIG)
    {
        gameMap[m][n].curState = BlockState::DIGGED;
        for(int y = -1; y <= 1; y++)
    ...
    if(gameMap[m][n].valueFlag == -1)
    {
        ...
        gameMap[m][n].curState = BlockState::BOMB;
    }
...
{
    // 如果标记错了，就记为错误标记，在 UI 层游戏结束时做绘制区分
    // 注意这里有个逻辑，如果一个方块标记两次会回到未挖掘的状态
    if(gameMap[m][n].curState == BlockState::UN_DIG)
    {
        if(gameMap[m][n].valueFlag == -1)
        {
            gameMap[m][n].curState = BlockState::MARKED;
        }
        else
        {
            gameState = GameState::FAULT;
            gameMap[m][n].curState = BlockState::WRONG_BOMB;
        }
        curMineNumber--; // 挖对了雷就减1
    }
    else if(gameMap[m][n].curState == BlockState::MARKED || gameMap[m][n].curState == BlockState::WRONG_BOMB)
    {
        gameMap[m][n].curState = BlockState::UN_DIG;
        gameState = GameState::PLAYING;
        curMineNumber++; // 雷数加回来
    }
    // 检查游戏输赢，并作调整
...
void GameModel::checkGame()
{
    ...
    if(gameState == GameState::OVER)
    {
        ...
        for(int i = 0; i < mRow; i++)
        {
            for(int j = 0; j < mCol; j++)
            {
                if(gameMap[i][j].valueFlag == -1)
                {
                    gameMap[i][j].curState = BlockState::BOMB;
                }
            }
        }
        return;
    ...
    // 如果雷排完了，且所有方块都挖出或者标记
    if(gameState != GameState::FAULT)
    {
        for(int i = 0; i < mRow; i++)
        {
            for(int j = 0; j < mCol; j++)
            {
                if(gameMap[i][j].curState == BlockState::UN_DIG)
                {
                    gameState = GameState::PLAYING;
                    return;
                }
