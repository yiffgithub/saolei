    ...
    for(int i = 0; i < game->mRow; i++)
    {
        for(int j = 0; j < game->mCol; j++)
        {
            ...
            case BlockState::BOMB:
                ...
                break;
            case BlockState::WRONG_BOMB:
                if(game->gameState == GameState::PLAYING || game->gameState == GameState::FAULT)
                {
                    // 如果还在游戏中就显示旗子
                    painter.drawPixmap(j * blockSize + offsetX, i * blockSize + offsetY + spaceY, bmpBlocks, blockSize * 11, 0, blockSize, blockSize);
                }
...
void MainGameWindow::handleGameState(GameModel *game)
{
    ...
    else if(game->gameState == GameState::WIN)
    {
        ...
        game->gameState = GameState::PLAYING; // 赶紧要改回正常状态，不然那个弹窗消失不了
    }
}
...
void MainGameWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    else if(game->gameState != GameState::OVER && game->gameState != GameState::WIN)
    {
        // 游戏没输或没赢才接受点击
        // 此时判断点击的是哪个方块
        // 获得点击坐标
...
void MainGameWindow::onLevelChooseClicked()
{
    ...
    if(actionSender == ui->actionBasic)
    {
        qDebug() << "basic";
        // 先设置游戏模型
        game->createGame(8, 10, 15, GameLevel::BASIC);
    }
    else if(actionSender == ui->actionMedium)
    {
        qDebug() << "medium";
        game->createGame(15, 20, 50, GameLevel::MEDIUM);
    }
    else if(actionSender == ui->actionHard)
    {
        qDebug() << "hard";
        game->createGame(20, 30, 100, GameLevel::HARD);
    }
    // 重新计时
    timer->start(1000);
    // 再刷新UI，窗口大小改变会强制重绘
